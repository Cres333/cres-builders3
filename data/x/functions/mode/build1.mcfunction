...
execute as @a[team=play] run scoreboard players operation @s Now = @s BuildPos1
function x:pos/tp/red
scoreboard objectives remove Now

## 少し後に第二建築エリアとお題エリアを再生成
schedule function x:area/green/generate 1s
schedule function x:area/title/generate 6s

## 持ち物
replaceitem entity @a[team=play] container.9 minecraft:barrier

## 各モードごとの設定
execute if score *mode Grobal matches 1 run gamemode spectator @p[team=play,scores={ID=1}]
